CREATE OR REPLACE TABLE `python-sandbox-381204.gkl2024_draft_analysis.pitcher_stats_agg` AS
...
WITH AvgQs AS (
  SELECT 
    ...
    AVG(GREATEST(CASE 
                 ...
                 END, 0)) AS avg_qs
  FROM 
    `python-sandbox-381204.gkl2024_draft_analysis.pitcher_stats`
  WHERE 
...
SELECT 
  ...
  eligible_positions, 
  MAX(cost) as actual_draft_cost,
  team_name
FROM 
  `python-sandbox-381204.gkl2024_draft_analysis.pitcher_stats` ps
JOIN 
  AvgQs aqs ON ps.IDfg = aqs.IDfg
WHERE 
  Season BETWEEN 2021 AND 2023
  AND eligible_positions IN ("SP","RP","P")
GROUP BY 
  ps.IDfg, Name, eligible_positions, team_name;

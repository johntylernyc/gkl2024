...
import numpy as np
from helpers import dataset_name, json_key_path, project_id

# Authenticate and create a BigQuery client
credentials = service_account.Credentials.from_service_account_file(json_key_path)
client = bigquery.Client(credentials=credentials, project=credentials.project_id)

table_name = "batter_adjusted_stats"
...
SELECT IDfg, Name, avg_ab, avg_r, avg_h, avg_3b, avg_hr, avg_rbi, avg_sb, avg_avg, hits_above_avg, avg_obp, obp_above_avg, avg_slg, slg_above_avg
    ...
    , stddev_OBP, stddev_SLG, SeasonsPlayed, LatestAge, eligible_positions, actual_draft_cost, team_name
...
df.to_gbq(batter_stats_consistency, project_id=project_id, if_exists='replace', credentials=credentials)
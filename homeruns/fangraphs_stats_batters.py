...
table_name = 'fangraphs_batter_stats'
...
def fangraphs_batter_stats(playerid, season):

   def fangraphs_overall_stats(playerid):
         url = f'https://www.fangraphs.com/api/players/stats?playerid={playerid}&position=&st=true&'
   ...
   def fangraphs_advanced_stats(playerid, season):
      url = f'https://www.fangraphs.com/api/players/splits?playerid={playerid}&position=&season={season}&split=&st=true&'
      response = requests.get(url)
      data = response.json()
      return data
   ...
   # Calculate AB per HR from the fields in overall_stats
   overall_stats['AB_per_HR'] = overall_stats['AB'] / overall_stats['HR']
   # Filter the DataFrame for the columns we want": ISO, BABIP, GB/FB, HR/FB, Pull%, Cent%, Oppo%, Soft%, Med%, Hard%, EV, LA, Barrel%, HardHit%, xSLG
   overall_stats = overall_stats[['Season', 'PA', 'HR', 'AB_per_HR', 'GB/FB', 'HR/FB', 'Pull%', 'Cent%', 'Oppo%', 'Soft%', 'Med%', 'Hard%', 'EV', 'LA', 'Barrel%', 'HardHit%']]
   ...
   split_stats = split_stats[['Split', 'PA', 'HR', 'GB/FB', 'FB%', 'Pull%', 'Cent%', 'Oppo%', 'Soft%', 'Med%', 'Hard%']]
   ...
   # Merge the two DataFrames on the playerid and season columns
   merged_df = pd.merge(overall_stats, split_stats, on=['playerid', 'season'])
   ...
   merged_df.to_csv('fangraphs_combined_player_stats.csv', index=False)

if __name__ == '__main__':
   # Ask the user for input
   playerid = input("Enter the playerid: ")
   season = input("Enter the season: ")
   # Store both playerid and season as integers
   playerid = int(playerid)
   season = int(season)
   # Call the function
   fangraphs_batter_stats(playerid, season)
   print("Pitcher stats saved to fangraphs_combined_batter_stats.csv")